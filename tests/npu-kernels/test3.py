"""Test spec for problem 3: Attention on the Spring 26 NPU.

Source kernel: sols/npu-kernels/3_attention_ref.py (generated by autocomp, 3630 cycles).

Computes unscaled dot-product attention:

  scores      = Q_pad @ K_pad              (matmul.mxu0, fp8×fp8→bf16)
  softmax_wts = softmax(scores, dim=-1)    (vexp + vrot.reduce.sum + vrcp + vmul)
  output      = softmax_fp8 @ V_pad        (matmul.mxu0; bf16 bytes reinterp as fp8)
...
Note on second matmul: matmul.mxu0 reads MRF as (64, 32) FP8.
After the softmax VPU pass the softmax tensor is BF16; those bytes are
reinterpreted as FP8 for the matmul. The golden result mirrors this.
"""
import os
import sys
import tempfile
...
class GeneratedProgram(Program):
    instructions: List[Instruction] = [
        ...
        Instruction("delay",         {"imm": 63}),

        # ── Softmax (VPU, all BF16) ───────────────────────────────────────────
        Instruction("vexp",            {"vrd": 2, "vs1": 1}),
        Instruction("delay",           {"imm": 7}),
        Instruction("vrot.reduce.sum", {"vrd": 3, "vs1": 2}),
        Instruction("delay",           {"imm": 7}),
        Instruction("vrcp",            {"vrd": 4, "vs1": 3}),
        Instruction("delay",           {"imm": 7}),
        Instruction("vmul",            {"vrd": 5, "vs1": 2, "vs2": 4}),
        Instruction("delay",           {"imm": 7}),
